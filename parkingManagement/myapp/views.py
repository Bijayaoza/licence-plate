...
def tflite_detect_images(modelpath, imgpath, lblpath, min_conf=0.4, txt_only=False):
    ...
    interpreter.set_tensor(input_details[0]['index'], np.expand_dims(image_resized, axis=0))
    interpreter.invoke()
    ...
    ocr_results_list = []
    ocr_confidences_list = []  # List to store confidence values corresponding to OCR results
    ...
    for i in range(len(scores)):
        if ((scores[i] > min_conf) and (scores[i] <= 1.0)):
            ...
            ocr_results_list.extend([result[1] for result in ocr_results])  # Append OCR text to the list
            ocr_confidences_list.extend([result[2] for result in ocr_results])  # Append confidence values to the list
    ...
    ocr_results_list = [text for text in ocr_results_list if 'nep' not in text.lower()]
...
def predict(request):
    ...
    if user_profile.tasbir:
        ...
        if os.path.exists(PATH_TO_MODEL) and os.path.exists(PATH_TO_LABELS):
            ...
            ocr_results, ocr_confidences, tensorflow_results = tflite_detect_images(PATH_TO_MODEL, file_path, PATH_TO_LABELS, min_conf_threshold)
            if not ocr_results:
                context = {
                'ocrText': 'boundary not detected ',  # Combine OCR text into a single string
                'ocrConfidences': ocr_confidences,  # Pass OCR confidences as a list
                'tensorflowResults': tensorflow_results
                }
                return render(request, 'myapp/ocr_result.html', context)
            # Prepare data to pass to the template
            context = {
                'ocrText': ' '.join(ocr_results),  # Combine OCR text into a single string
                'ocrConfidences': ocr_confidences,  # Pass OCR confidences as a list
                'tensorflowResults': tensorflow_results
            }
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            if User.objects.filter(username=username).exists():
                ...
                messages.success(request, 'Staff registered successfully: ' + username)
                return HttpResponseRedirect('/sucess/reg/')
        else:
            messages.error(request, 'Passwords do not match or other form errors!')
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            if len(vehicle_no) < 8:
                ...
                form.add_error('vehicle_no', 'Vehicle number must be at least 8 characters long.')
            else:
                ...
                try:
                    ...
                    park_entry = Paaark.objects.get(modifiednoplate__iexact=clean_vehicle_no)
                    print(park_entry)
...
def add_park(request):
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            if len(vehicle_no) < 8:
                # If the vehicle number is too short, add an error to the form
                form.add_error('noplate', 'Vehicle number must be at least 8 characters long.')
            else:
                # Using __iexact for case-insensitive exact match
                clean_vehicle_no = ''.join(vehicle_no.split()).lower()
                try:
                    ...
                    park_entry = Paaark.objects.get(modifiednoplate__iexact=clean_vehicle_no)
                    form.add_error('noplate', 'Vehicle plate is already registered.')
                except Paaark.DoesNotExist:
                    form.save()
                    messages.success(request, 'Vehicle details registered successfully.')
                    return redirect('transport')  # Redirect to search results page after saving
    else:
        form = PaaarkForm()
